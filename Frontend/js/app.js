    ...
    cy.on('tap', 'node', function (e) {
        ...
        if (tappedTimeout && tappedBefore === n) {
            ...
            tappedTimeout = setTimeout(function () {
                ...
                fetch(`http://localhost:3000/graph/expand/${encodeURIComponent(nodeId)}`)
                    .then(r => r.json())
                    .then(newData => {
                        loader.style.display = 'none';
...
function expandToLevel2(node) {
    ...
    const level2Promises = currentNeighbors.nodes().map(neighbor =>
        fetch(`http://localhost:3000/graph/expand/${encodeURIComponent(neighbor.id())}`)
            .then(r => r.json())
            .catch(() => ({ nodes: [], edges: [] }))
    );
...
chatForm.onsubmit = async (e) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/chat?message=${encodeURIComponent(msg)}`);
